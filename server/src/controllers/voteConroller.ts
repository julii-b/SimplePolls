...
export const castVote = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!Number.isInteger(answerId)) throw HttpError.badRequest('answerId must be an integer');
    // cast vote:
    const newVote: voteRepository.Vote|null = await voteRepository.vote(req.userId, answerId);
